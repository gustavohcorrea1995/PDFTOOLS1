...
app.post('/api/ocr', upload.single('file'), async (req, res) => {
  if(!req.file) return res.status(400).json({error:'Envie um PDF ou uma imagem.'});

  const inputPath = req.file.path;
  const id = uuid();
  const workDir = path.join(TMP, 'ocr_' + id);
  const outputPath = path.join(workDir, 'ocr.pdf');

  try{
    fs.mkdirSync(workDir, { recursive:true });

    // OCRmyPDF cria uma camada de texto pesquisável sem mudar
    // visualmente o documento original.
    await run('ocrmypdf', [
      '--force-ocr',
      '--deskew',
      '--rotate-pages',
    ...
    if(!fs.existsSync(outputPath)){
      throw new Error('O OCR não gerou o PDF de saída.');
    }

    res.download(outputPath, 'pdf-pesquisavel.pdf', err => {
      cleanup(inputPath, workDir);
      if(err) console.error('OCR download:', err);
    });
  }catch(e){
    ...
    if(!res.headersSent){
      res.status(500).json({
        error:'Não foi possível executar o OCR: ' + e.message
      });
    }
  }
});
