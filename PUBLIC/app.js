...
RENDERERS['ocr'] = (root)=>{
  const dz = makeDropzone(root, {
    ...
    multiple: false,
    label: 'Arraste um PDF escaneado ou uma imagem'
  });

  const hint = document.createElement('p');
  hint.className = 'hint';
  hint.textContent =
    'O OCR mantém a aparência do documento e adiciona uma camada de texto pesquisável. Depois, abra o PDF gerado no editor para editar ou excluir textos.';
  root.appendChild(hint);

  const btn = makeButton(root, 'Executar OCR');
  btn.dataset.label = btn.textContent;

  btn.onclick = async ()=>{
    const files = dz.getFiles();
    if(files.length !== 1) return toast('Selecione um PDF ou imagem.', true);

    const fd = new FormData();
    fd.append('file', files[0]);

    setLoading(btn, true, 'Reconhecendo texto…');

    try{
      const res = await postForm('/api/ocr', fd);
      const blob = await res.blob();
      downloadBlob(blob, 'pdf-pesquisavel.pdf');
      toast('OCR concluído! O PDF agora pode ser pesquisado e editado.');
    }catch(e){
